 ...
 */
	...
	for (INT threads = 0; threads < workThreads; threads++)
	{
		...
		para[threads].batch_h = batch_h; // head entities batch
		para[threads].batch_t = batch_t; // tail entities batch
		para[threads].batch_r = batch_r; // relations batch
		para[threads].batch_y = batch_y; // round truth batch
		para[threads].batchSize = batchSize; // batch size
		para[threads].negRate = negRate; // negative entities rate
		para[threads].negRelRate = negRelRate; // regative relations rate
		para[threads].p = p; // p-norm
		para[threads].val_loss = val_loss; // ???
		para[threads].mode = mode; // mode = 0: sampling head and tail; mode = -1: sampling head; mode = 1: sampling tail
